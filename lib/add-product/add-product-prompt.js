...
const productPrompt = function(promptObj) {
  return new Promise((res, rej) => {  // wrapped in a promise so it waits for an entry before continuing
    prompt.get(promptObj, function(err, prod) {
      ...
      res(prod.selected in productIds ? prod.selected : 0)  // the return will be the number of the selected item
    })
  })
}
...
getProducts()  // making a sql SELECT for all the products
  .then((data) => {
    data.forEach( (each) => {  // building up a string of all the products to add to the prompt line
        allProducts +=`
    ...
    productPrompt(promptObj)  // running the prompt
  })


module.exports = { productPrompt, setProductsPrompt }
